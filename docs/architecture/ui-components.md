...
- **Stable-height contract:** `#wizardTabContent` is a CSS grid and every `.tab-pane` sits in the same cell (`grid-area: 1 / 1`), so the browser sizes the container to the tallest pane on every reflow; inactive panes stay in layout but are `visibility: hidden` + `pointer-events: none` (which also keeps them out of the a11y tree and the tab order). `.wizard-nav` carries `margin-top: auto`. **Both are required** — the grid stops the container resizing, the auto margin stops the buttons floating up inside a short step. Removing either reintroduces the vertical jump between steps. Do not put the panes back on `display: none`: that takes them out of the grid and the cell collapses to the active pane. An earlier revision measured the panes in JS (`measurePanes()`) and wrote an inline `min-height`; it read ~16px short of the tallest pane and needed a resize listener, and was replaced by the grid.

  Holding the *container's* height is necessary but not sufficient: the nav row's own height must not vary either, because pinning it to the bottom means anything that makes it taller moves its buttons' **tops** up. Two rules in `_setup-wizard.scss` enforce that. `.wizard-nav` is always `display: flex` — as a block box, a single `text-end` button is an inline-block on a baseline and the descender space lifts it ~2px above the flex steps' buttons. And `.wizard-nav .btn` is `white-space: nowrap` — in the real ~270px middle column `Review & Download` wrapped to a second line, which moved step 4's button a full line-height (16px) off everyone else's. Keep labels short enough to fit rather than relaxing either rule; measurements in `test/visual/evidence/setup-wizard-nav-alignment/`.
- **Tests:** `test/visual/features/setup-wizard.spec.js` — stepper replaces nav-pills, preview live at every step, Back/Next y-offset stable across steps, blur validation, lock/unlock, Review warnings, draft round-trip through reload and clear-on-download, a still-pending draft write surviving an immediate reload (the `pagehide` flush), and survival when `localStorage` throws.
