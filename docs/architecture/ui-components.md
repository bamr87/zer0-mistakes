...
- **API surface:** ids `#navbar`, `#main-content`, `#top-progress-target`; classes `.navbar-main`, `.navbar-main-start`, `.navbar-brand-group`, `.navbar-home-links`, `.navbar-utility-controls`, `.bottom-shadow`, `.bd-navbar-toggle`, `.navbar-toggler-labeled`, `.navbar-hidden`; container `container-name: navbar-main`; data-attr `data-bs-target` (toggles offcanvas/drawer); CSS vars `--bs-body-bg`, `--zer0-bp-*`, `--zer0-navbar-max-width` (consumer opt-in cap on the bar width), `--zer0-navbar-side-min` (default `9rem`; shared floor on the lg+ grid's brand/utility side tracks so the menubar is optically centred — #405)
...
- **Purpose:** Renders the clickable site title inside the brand cluster, with responsive icon and ellipsis behavior. The navbar carries logo + title ONLY (#405) — `site.subtitle`, when configured, renders on the home hero (`_layouts/home.html`) instead, where it no longer competes with the menubar for bar width.
- **Capabilities:** Title links to `/`; optional title icon shown only at sm–lg (`d-none d-sm-inline d-lg-none`); `site-title-text` ellipsis at viewport breakpoints (60vw → 50vw → 40vw).
- **Source:**
  - SCSS: `_sass/core/_navbar.scss` (`.site-title-text` responsive `max-width`, `#navbar .site-title` ellipsis in the lg grid)
  - Markup: `_includes/core/branding.html` (title); `_layouts/home.html` (subtitle, home hero only)
- **API surface:** classes `.site-title`, `.site-title-text`, `.navbar-brand`, `.home-subtitle`; config `site.title`, `site.subtitle`, `site.title_icon`, `site.default_icon`
- **Tests:** `test/visual/features/navbar.spec.js` — "mid desktop brand logo and title do not overlap" (asserts `.site-title-text` sits right of the logo, no overlap); "mobile shows site title and offcanvas toggler" (asserts `.site-title-text` visible). `test/visual/features/navbar-tiers.spec.js` — "the navbar carries logo + title only — no subtitle" and "the subtitle moved to the home hero". `test/visual/core/styling.spec.js` checks the `a.navbar-brand` logo image exists.
- **Gaps / improvement ideas:** The title-icon breakpoint window (sm–lg only) is untested. There is a stale `d-lg-none` icon class string that duplicates `site.default_icon` (`{{ site.default_icon }} {{ site.default_icon }}-...`) — likely a typo that should be `{{ site.default_icon }} bi-{{ site.title_icon }}`; worth verifying the icon actually renders.
...
- **Capabilities:** Data-driven from `_data/navigation/main.yml`, else auto-generated from `site.collections` (skips `pages`, honors `nav_exclude`); split dropdown (parent link navigates, chevron button — merged into the link's own hover row and padding box, #405 — toggles submenu); `dropdown-menu-end` alignment for last-two items; icon+label with a TWO-tier container query on the `bd-nav` track (icon+label ≥51rem → icon-only <51rem; icons survive both tiers, no `text-overflow: ellipsis` — a label is whole or absent, never "Quicksta…"); active state via `aria-current="page"` with an underline pseudo-element; mobile offcanvas adds Home/Search/Settings entries; tooltips on compact desktop (992–1199px).
- **Source:**
...
- **Tests:** `test/visual/features/navbar.spec.js` — "wide desktop shows full nav labels without ellipsis" (no `...` suffix; "Quick Start" visible); "no nav label is CSS-truncated at any desktop width"; dropdown open/close, outside-click-close, Escape-close; "mobile shows site title and offcanvas toggler". `test/visual/features/navbar-tiers.spec.js` — the two-tier container query resolves at both ends, icons survive both tiers, no dead zone between a dropdown's label and its merged chevron, the chevron keeps its ARIA, exactly two toggles below lg and the labelled one fits its own box. `test/visual/core/styling.spec.js` — "mobile main navigation toggle is visible" asserts `.bd-navbar-toggle.d-lg-none button.navbar-toggler[data-bs-target="#bdNavbar"]`.
- **Gaps / improvement ideas:** Keyboard arrow/Home/End navigation within an open dropdown is untested. Tooltip show/hide on compact desktop is untested. No test exercises the auto-generated (no `main.yml`) nav fallback path.
