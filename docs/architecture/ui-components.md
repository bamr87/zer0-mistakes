...
|---|---|---|---|
| Header / Site Shell | `_includes/core/header.html` | `styling.spec.js`, `core/responsive.spec.js`, `core/layout-chrome.spec.js` | 🟡 partial |
| Branding (title/subtitle) | `_includes/core/branding.html` | `features/navbar.spec.js`, `styling.spec.js` | 🟡 partial |
| Primary Navbar (menubar + dropdowns) | `_includes/navigation/navbar.html` | `features/navbar.spec.js`, `styling.spec.js` | 🟡 partial |
...
| Component | Primary source | Test | Coverage |
|---|---|---|---|
| Docs layout shell (.bd-layout/.bd-sidebar/.bd-main/.bd-toc) | `_sass/core/_docs-layout.scss` | `styling.spec.js`, `features/content-enhancements.spec.js` | 🟡 partial |
...
| Page intro header (.bd-intro family) | `_includes/content/intro.html` | `features/layouts.spec.js`, `layouts.spec.js` | 🟡 partial |
...
| Bootstrap component polish (UI enhancements) | `_sass/components/_ui-enhancements.scss` | `features/layouts.spec.js`, `features/content-enhancements.spec.js`, `core/layout-chrome.spec.js`, `core/responsive.spec.js` | 🟡 partial |
...
| Code copy button | `assets/js/code-copy.js` | `features/content-enhancements.spec.js`, `accessibility.spec.js` | 🟡 partial |
...
- **Tests:** `test/visual/styling.spec.js` — "desktop header and navbar render" (asserts `header#navbar`, `.navbar.navbar-expand-lg`, `nav.navbar-main` visible; brand link + img present); "mobile main navigation toggle is visible". `test/visual/core/responsive.spec.js` — "main landmarks visible at {viewport}" asserts `header#navbar` visible across all 5 viewports. `test/visual/core/layout-chrome.spec.js` — "skip link is focusable" asserts `a[href="#main-content"].visually-hidden-focusable` is attached + focusable.
...
- **Tests:** `test/visual/features/navbar.spec.js` — "mid desktop brand logo and title do not overlap" (asserts `.site-title-text` sits right of the logo, no overlap); "mobile shows site title and offcanvas toggler" (asserts `.site-title-text` visible). `test/visual/styling.spec.js` checks the `a.navbar-brand` logo image exists.
...
- **Tests:** `test/visual/features/content-enhancements.spec.js` — "docs article exposes ToC on desktop" (asserts `.bd-toc, #tocContents` attached; `.bd-sidebar-visibility-toggle` / `.bd-toc-visibility-toggle` attached). The Bootstrap offcanvas open/close itself is exercised indirectly via the toggler tests in styling.spec.js.
...
- **Tests:** No automated tests (the generic skip-link focus test in `test/visual/core/layout-chrome.spec.js` is unrelated).
...
- **Tests:** No automated tests target `.bd-example`/`.bd-clipboard`/AnchorJS. (The `code-blocks` specs in `test/visual/features/content-enhancements.spec.js` test a *different* system — `.code-block-header`/`.code-line-numbers` from the rouge enhancer, not `docs.min.js`'s `.bd-clipboard`.)
...
- **Tests:** `test/visual/features/layouts.spec.js` — "intro action buttons share consistent height". `test/visual/features/content-enhancements.spec.js` — "landing comparison table has toolbar and distinct header" (thead bg ≠ tbody bg) and the code-block header/copy tests. `test/visual/core/layout-chrome.spec.js` — footer-link checks. `test/visual/core/responsive.spec.js` — axe advisory scans per viewport. No spec asserts the ripple element, card hover transform, badge hover, or the `window.zer0UI` toast/clipboard API.
...
- **Tests:** No spec instantiates or asserts post-card structure/badges. (Used on `/tags/` and `/news/` which appear only indirectly in `test/visual/features/layouts.spec.js` section/grid checks that don't reach card internals.)
...
| `core/layout-chrome.spec.js` | Global chrome: footer powered-by links + equal-width nav columns, header skip-to-content link focusability | 3 | smoke |
| `core/responsive.spec.js` | Cross-viewport smoke: homepage axe advisory scan and key-landmark (header/main/footer) visibility | 2 (×5 viewports) | smoke |
| `features/appearance.spec.js` | Theme preview gallery renders; theme-customizer skin-card click applies live `--bs-primary` + YAML export | 2 | smoke |
| `features/category-badges.spec.js` | Features page category badges link to in-page anchors | 1 | smoke |
| `features/content-enhancements.spec.js` | Rouge code-block header+gutter, content-table CSV toolbar, docs ToC/sidebar-FAB chrome, code-copy click→clipboard write+feedback | 6 | smoke |
| `features/layouts.spec.js` | Intro hero meta-footer/button-height/aria-label, section-archive layout containment | 4 | smoke |
| `features/navbar.spec.js` | Navbar label ellipsis/brand overlap/mobile quicklinks, dropdown chevron open/close + Esc + outside-click | 7 | smoke |
| `features/search.spec.js` | Search modal open→type→results→close, toggle-button open, no-results message, close button | 4 | smoke |
...
- ✅ **Search modal / `search.json`** — **now covered** by `test/visual/features/search.spec.js` (open via `/` + toggle button, type → results, no-match message, close). Remaining: arrow-key result navigation. *(was: zero coverage)*
...
- ✅ **Code-copy click → clipboard write** — **now covered** by `test/visual/features/content-enhancements.spec.js` (grants clipboard perms, clicks copy, asserts `.copied` feedback + non-empty `clipboard.readText()` + revert). *(was: only focusable; the `config-viewer.spec.js` clipboard test is still `test.fixme`)*
- ✅ **Navbar dropdown open/close** — **now covered** by `test/visual/features/navbar.spec.js` (chevron click open/close, Escape-to-close, outside-click-close, `aria-expanded`). Remaining: arrow/Home/End item navigation. *(was: layout/visibility only)*
- ✅ **Theme customizer "apply" (skin)** — **now covered** by `test/visual/features/appearance.spec.js` (click skin swatch → live `data-theme-skin` + `--bs-primary` change + YAML export). Note: color **pickers** are export-only by design (they do not live-apply). *(was: API-only)*
...
**Coverage summary:** 🟢 4 good · 🟡 47 partial · 🔴 60 none (of 111 components, as inventoried). *Note: the interactions added after this sweep — search modal, code-copy, navbar dropdowns, theme-customizer skin apply — were originally landed together in `test/visual/interactions.spec.js`; that grab-bag file has since been dissolved into `test/visual/features/{search,content-enhancements,navbar,appearance}.spec.js` respectively. They lift 4 high-priority surfaces into behavioral coverage — see the gaps list below.*
