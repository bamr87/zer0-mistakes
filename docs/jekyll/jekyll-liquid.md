...
```html
{%- raw -%} {% for product in collections.frontpage.products %} {% if
...
```html
{%- raw -%} {% assign my_variable = "tomato" %} {{ my_variable }} {% endraw %}
```
...
```html
{%- raw -%} {% for product in collection.products %} {{ product.title }} {%
endfor %} {% endraw %}
```
...
{% for item in array limit:2 %} {{ item }} {% endfor %} {% endraw %}
```
