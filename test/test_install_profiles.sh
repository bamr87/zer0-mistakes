...
# test_install_profiles.sh — Validate every install profile end-to-end.
# =============================================================================
# For each profile listed by `install list-profiles`, scaffold into a temp
# workspace and assert the file tree matches the profile's `includes:` list.
# =============================================================================
...
# ---- per-profile expectations (paths relative to target) --------------------
# Keep these tight — only assert the *signature* files for each profile.
declare -a EXPECTED_minimal=("Gemfile" "_config.yml" "index.md" ".gitignore")
declare -a EXPECTED_full=(
    "Gemfile"
    "_config.yml"
...
declare -a EXPECTED_github=(
    "Gemfile"
    "_config.yml"
    ".gitignore"
)
declare -a EXPECTED_fork=(
    "Gemfile"
    "_config.yml"
    ".gitignore"
)
# remote requires network — opt in via INSTALL_TEST_INCLUDE_REMOTE=1
declare -a EXPECTED_remote=("Gemfile" "_config.yml")
...
run_profile_test() {
    ...
    test_log_test "Installing profile: $profile → $workspace"

    set +e
    "$INSTALL_BIN" init --profile "$profile" --force "$workspace" >/tmp/install-$$-stdout 2>/tmp/install-$$-stderr
    local rc=$?
    set -e

    if [[ $rc -ne 0 ]]; then
        test_log_error "install init --profile $profile failed (rc=$rc)"
        sed -n '1,40p' /tmp/install-$$-stderr || true
        rm -f /tmp/install-$$-stdout /tmp/install-$$-stderr
        return 1
    fi
    rm -f /tmp/install-$$-stdout /tmp/install-$$-stderr
    ...
    for path in "${expected[@]}"; do
        if [[ ! -e "$workspace/$path" ]]; then
            test_log_error "  Missing expected: $path"
            missing=$((missing + 1))
        fi
    done

    if [[ $missing -gt 0 ]]; then
        test_log_error "Profile $profile: $missing expected paths missing"
        return 1
    fi

    test_log_success "Profile $profile produced expected file tree"
    return 0
}
...
main() {
    ...
    if [[ ! -x "$INSTALL_BIN" ]]; then
        test_log_error "Installer not found or not executable: $INSTALL_BIN"
        exit 127
    fi

    local -a profiles=(minimal full github fork)
    if [[ "${INSTALL_TEST_INCLUDE_REMOTE:-0}" == "1" ]]; then
        profiles+=(remote)
    else
        test_log_skip "Skipping 'remote' profile (set INSTALL_TEST_INCLUDE_REMOTE=1 to enable)"
    fi

    for profile in "${profiles[@]}"; do
        local var="EXPECTED_${profile}[@]"
        run_test "profile_${profile}" \
            "run_profile_test '$profile' \"\${$var}\"" \
            "profiles" || true
    done

    echo ""
    test_log_info "================================================================"
    test_log_info "  Results: $INSTALL_TESTS_PASSED passed / $INSTALL_TESTS_TOTAL total"
    test_log_info "================================================================"
