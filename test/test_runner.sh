...
TEST_SUITE_KEYS=("core" "deployment" "quality" "installation" "site_generation" "obsidian" "playwright" "playwright_snapshots")
TEST_SUITE_SCRIPTS=("test_core.sh" "test_deployment.sh" "test_quality.sh" "test_installation.sh" "test_site_generation.sh" "test_obsidian.sh" "test_playwright.sh" "test_playwright.sh")
TEST_SUITE_NAMES=("Core Tests (Unit, Integration, Validation)" "Deployment Tests (Installation, Docker, E2E)" "Quality Tests (Security, Accessibility, Compatibility, Performance)" "Installation Tests (CLI, Modes, Errors, Edge Cases)" "Site Generation Tests (Config Matrix, Jekyll Build)" "Obsidian Tests (Wiki Links, Graph, Backlinks)" "Playwright Smoke Tests (CSS, layout, behavioral DOM)" "Playwright Snapshot Tests (homepage skin pixel regression)")
