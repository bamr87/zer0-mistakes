/**
 * Core cross-cutting responsive baseline.
 *
 * Combines two guards that both live at the "does the layout survive real
 * viewports" altitude:
 *
 *  1. Mobile tap-targets / overflow / FAB-stacking — runs under real phone
 *     emulation (Pixel 5 profile: mobile viewport, touch, mobile UA) —
 *     unlike the desktop-window specs, this exercises the mobile
 *     layout-viewport behaviour where ANY uncontained content overflow
 *     widens the initial containing block, so the fixed header renders
 *     wider than the visual viewport and the page pans sideways ("navbar
 *     looks cut off"). Guards the defects found in the 2026-07 mobile audit:
 *
 *       a. Author card ("About the Author"): a long nowrap expertise badge
 *          used to propagate through flexbox min-width:auto and push the
 *          page past the viewport on small phones (≤340px).
 *       b. Cookie consent banner: the .zer0-bg-body elevation rule flattened
 *          the banner (and #tocFab) to z-index 1, so the chat / local-graph
 *          FABs painted over the banner and stole taps from its
 *          Reject/Accept buttons.
 *       c. Footer link tap targets: text links ~19px tall and icon-only
 *          social links ~16px wide were under the WCAG 2.5.8 24px minimum.
 *       d. Code-copy buttons: ~22px tall; must be ≥24px (≥32px on
 *          coarse/touch pointers).
 *       e. Navbar chrome (offcanvas menu, search modal) stays within the
 *          viewport and its controls remain tappable at phone sizes.
 *
 *  2. Landmark visibility — header#navbar / main / footer.bd-footer stay
 *     visible across the full desktop/tablet/mobile viewport matrix
 *     (VIEWPORTS from fixtures), independent of phone emulation.
 *
